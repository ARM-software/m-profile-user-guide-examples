...
$COMPILER/armclang --target=arm-arm-none-eabi -march=armv8.1-m.main+mve.fp+fp.dp -mfloat-abi=hard -mthumb -mlittle-endian -mcmse -O1 -g -mfpu=none -c basic-Non-secure-only-program_s/main_s.c -o logs/main_s.o -DARMv81MML_DSP_DP_MVE_FP $INCLUDES_S
...
$COMPILER/armclang --target=arm-arm-none-eabi -march=armv8.1-m.main+mve.fp+fp.dp -mfloat-abi=hard -mlittle-endian -O1 -g -mfpu=none -c basic-Non-secure-only-program_ns/main_ns.c -o logs/main_ns.o -DARMv81MML_DSP_DP_MVE_FP $INCLUDES_NS
...
$COMPILER/armlink --scatter=$SCAT_FILE_S --entry=Reset_Handler --info=sizes logs/startup_s.o logs/system_s.o logs/main_s.o -o logs/basic-Non-secure-only-program_s.axf --list=logs/image_s.lst --map
echo "Generating secure binary file..."
$COMPILER/fromelf --bincombined logs/basic-Non-secure-only-program_s.axf -o logs/basic-Non-secure-only-program_s.bin
$COMPILER/fromelf -c logs/basic-Non-secure-only-program_s.axf -o logs/basic-Non-secure-only-program_s.disass
echo "Linking non-secure image..."
$COMPILER/armlink --scatter=$SCAT_FILE_NS --entry=Reset_Handler --info=sizes logs/startup_ns.o logs/system_ns.o logs/main_ns.o -o logs/basic-Non-secure-only-program_ns.axf --list=logs/image_ns.lst --map
echo "Generating secure binary file..."
$COMPILER/fromelf --bincombined logs/basic-Non-secure-only-program_ns.axf -o logs/basic-Non-secure-only-program_ns.bin
$COMPILER/fromelf -c logs/basic-Non-secure-only-program_ns.axf -o logs/basic-Non-secure-only-program_ns.disass
